...
for col in gpfinal.columns:
    if (col in list(test.columns)):
          print '-'
    else:
        if (col != 'date'):
            gpfinal.drop(col, axis=1, inplace=True)  

for col in test.columns:
    if (col in list(gpfinal.columns)):
          print '-'
    else:
        if (col != 'date'):
...
test['stockAux'] = 0
gpfinal['stockAux'] = 0
test['stockAux'][test['stock'] == 1] = 1
test['stockAux'][test['stock'] > 1] = 2
test['stockAux'][test['stock'] > 6] = 3
gpfinal['stockAux'][gpfinal['stock'] == 1] = 1
gpfinal['stockAux'][gpfinal['stock'] > 1] = 2
gpfinal['stockAux'][gpfinal['stock'] > 6] = 3

print gpfinal['stockAux']
